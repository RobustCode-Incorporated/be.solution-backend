  ...
  module.exports = {
    // Liste toutes les demandes avec détails complets, y compris type de document
    async getAllDemandes(req, res) {
      try {
        const demandes = await Demande.findAll({
          include: [
            { model: Citoyen, as: 'citoyen', include: [{ model: Commune, as: 'commune' }] },
    ...
    async createDemande(req, res) {
      try {
        // Harmonisation pour Robust Code: validation du type de document et statut initial
        if (!req.body.typeDemande) {
          return res.status(400).json({ message: "Le type de document est requis (typeDemande)." });
        }
        if (req.body.donneesJson && typeof req.body.donneesJson !== 'string') {
          req.body.donneesJson = JSON.stringify(req.body.donneesJson);
        }
        // Statut initial: "en attente"
        const statutEnAttenteId = await getStatutIdByName('en attente');
        if (!statutEnAttenteId) {
          return res.status(500).json({ message: "Statut 'en attente' non trouvé." });
        }
    ...
    async updateDemande(req, res) {
      try {
        ...
        if (!demande) return res.status(404).json({ message: 'Demande non trouvée' });
        // Harmonisation: empêcher modification du statut ici, sauf via workflow
        const { statutId, ...updateFields } = req.body;
        await demande.update(updateFields);
        res.json(demande);
      } catch (error) {
    ...
    async deleteDemande(req, res) {
      try {
        const demande = await Demande.findByPk(req.params.id);
        if (!demande) return res.status(404).json({ message: 'Demande non trouvée' });
        // Suppression uniquement si statut 'en attente' ou 'rejetée'
        const statut = await Statut.findByPk(demande.statutId);
        if (!['en attente', 'rejetée'].includes(statut.nom)) {
          return res.status(403).json({ message: "Suppression non autorisée à ce statut." });
        }
    ...
    async getDemandesToValidate(req, res) {
      try {
        // Pour validation, on prend les statuts 'en traitement'
        const demandes = await Demande.findAll({
          include: [
            { model: Citoyen, as: 'citoyen', include: [{ model: Commune, as: 'commune' }] },
    ...
    // Génération du document selon le type, avec QR code et statut cohérent
    async generateDocument(req, res) {
      ...
      try {
        ...
        const verificationUrl = `http://localhost:4001/verify-document?token=${verificationToken}`;
  ...
  <body>
    <div class="header-with-image">
      <img src="http://localhost:4001/assets/images/app_logo.png" alt="Logo" class="header-image">
    ...
    <div class="footer-line"></div>
        ...
        // Passage au statut "en traitement"
        const enTraitementStatutId = await getStatutIdByName('en traitement');
        await demande.update({
          documentPath: filename,
          verificationToken: verificationToken,
          statutId: enTraitementStatutId
        });
        console.log('Demande mise à jour en base de données avec documentPath, verificationToken et statut "en traitement".');
        res.json({
        ...
        // En cas d'échec de la génération, mettez explicitement documentPath à null et statut à "en attente"
        const statutEnAttenteId = await getStatutIdByName('en attente');
        await demande.update({
          documentPath: null,
    ...
    // Validation et signature du document, passage au statut "validée"
    ...
    // Téléchargement du document validé (seulement si validé)
    async downloadDocument(req, res) {
      try {
        ...
        // Vérification du statut validée
        if (demande.statut.nom !== 'validée') {
          return res.status(403).json({ message: "Seuls les documents validés peuvent être téléchargés." });
        }
    ...
    async getAllStatuts(req, res) {
      try {
        const statuts = await Statut.findAll({ order: [['id', 'ASC']] });
